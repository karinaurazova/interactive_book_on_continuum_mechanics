...
const text = {
  ru: {
    title:'Нео–Гук и Муни–Ривлин: что меняет I₂',
    ...
    keyText:'Модель Нео–Гука (Neo-Hookean) зависит только от I₁, а модель Муни–Ривлина (Mooney–Rivlin) — от I₁ и I₂. При одинаковом малодеформационном модуле различие проявляется по мере роста деформации и зависит от протокола.',
    models:'Две модели',
    ...
    mr:'P, Муни–Ривлин',
    diff:'разница',
    sceneKicker:'СРАВНЕНИЕ МОДЕЛЕЙ',
    sceneTitle:'одинаковый μ, разные зависимости от инвариантов',
    checkpoint:'ВОПРОС ДЛЯ ПРОВЕРКИ',
    checkpointTitle:'Почему одна удачная аппроксимация ещё не доказывает корректность модели?',
    ...
    conclusionText:'Следующий шаг — посмотреть семейство Огдена (Ogden), где энергия задаётся напрямую через главные растяжения и позволяет гибко описывать сильную нелинейность.',
    deepen:'Углубиться',
    ...
    researchText:'Практическая идентифицируемость параметров зависит не только от числа параметров модели, но и от информативности экспериментальных путей. Это прямой мост к планированию эксперимента и обратной идентификации параметров (inverse modeling).',
    warning:'ВАЖНО',
      ...
      <div className="scene-column">
        <div className="scene-card">
          ...
          <svg className="balance-scene" viewBox="0 0 100 60" role="img">
            ...
            <text x="11" y="13" fill="#2864FF" fontSize="2.3">{language==='ru'?'Нео–Гук':'Neo-Hookean'}</text>
            <text x="11" y="17" fill="#A9E3D2" fontSize="2.3">{language==='ru'?'Муни–Ривлин':'Mooney–Rivlin'}</text>
