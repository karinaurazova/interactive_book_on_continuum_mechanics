...
key:'КОНСТИТУТИВНАЯ МОДЕЛЬ В FEM — ЭТО ЛОКАЛЬНЫЙ РЕШАТЕЛЬ СОСТОЯНИЯ',
keyText:'Глобальный FEM передаёт в материальную точку новую деформацию; локальный constitutive update возвращает напряжение, обновлённые внутренние переменные и алгоритмическую касательную.',
update:'Дискретное обновление',
updateText:'Для простой внутренней переменной q с законом q̇=(g(ε)−q)/τ explicit-схема использует правую часть в момент n, а implicit-схема требует решить нелинейное или линейное локальное уравнение на qₙ₊₁.',
residual:'Локальный остаток и Newton',
residualText:'Implicit update удобно записывать как R(qₙ₊₁)=0. Если R нелинеен, выполняются локальные итерации Ньютона до сходимости. Это происходит независимо в каждой интеграционной точке.',
tangent:'Алгоритмическая касательная',
tangentText:'После локального обновления нужен производный отклик dσₙ₊₁/dεₙ₊₁. Именно эта алгоритмическая касательная входит в глобальный Newton FEM и определяет его скорость сходимости.',
sceneKicker:'LOCAL CONSTITUTIVE SOLVER',
sceneTitle:'Сравни explicit и implicit update на одном шаге времени',
scheme:'схема',explicit:'explicit',implicit:'implicit',
dt:'шаг Δt',tau:'время τ',epsn:'εₙ',eps1:'εₙ₊₁',qn:'qₙ',alpha:'нелинейность α',
q1:'qₙ₊₁',sigma:'σₙ₊₁',res:'|R|',iters:'итерации',tangentValue:'dσ/dε',
warning:'ВАЖНО',
warningTitle:'Устойчивость local update и сходимость global Newton — разные вещи.',
...
deepen:'Углубиться',
deepenText:'В реальной конечнодеформационной модели вместо скаляра q хранится тензорная внутренняя переменная, например Fᵥ, Cᵥ или несколько branch-states. Локальный residual становится тензорным, но структура алгоритма остаётся той же: trial state → local solve → stress → consistent tangent.',
...
keyText:'The global FEM solver supplies a new deformation to a material point; the local constitutive update returns stress, updated internal variables, and the algorithmic tangent.',
...
tangentText:'After the local update we need dσₙ₊₁/dεₙ₊₁. This algorithmic tangent enters the global FEM Newton method and strongly affects convergence.',
sceneKicker:'LOCAL CONSTITUTIVE SOLVER',
sceneTitle:'Compare explicit and implicit updates over one time step',
scheme:'scheme',explicit:'explicit',implicit:'implicit',
...
deepen:'Go deeper',
deepenText:'In a finite-strain model the scalar q is replaced by tensor-valued variables such as Fᵥ, Cᵥ, or several branch states. The local residual becomes tensorial, but the algorithmic structure is unchanged: trial state → local solve → stress → consistent tangent.',
...
{ru:'FEniCSx и собственные constitutive kernels',en:'FEniCSx and custom constitutive kernels'},
{ru:'Нелинейный FEM мягких тканей',en:'Nonlinear FEM of soft tissues'},
