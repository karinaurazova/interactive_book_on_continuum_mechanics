...
experimentsText:'Тест релаксации напрямую зондирует спад G(t); частотный проход разделяет E′(ω) и E″(ω). Ползучесть и циклические тесты добавляют другую комбинацию чувствительностей. Совместная идентификация по нескольким протоколам обычно устойчивее.',
...
warning:'ВАЖНО',
warningTitle:'Малый остаток не гарантирует уникальности параметров.',
warningText:'Оптимизатор может найти очень хорошую подгонку вдоль длинной плоской долины функции ошибки. В таком случае прогноз внутри окна калибровки может быть хорошим, а параметры и экстраполяция — ненадёжными.',
...
conclusionText:'Следующий шаг — собрать всю главу в вычислительную лабораторию: выбрать модель, задать эксперимент, провести моделирование и выполнить физические проверки корректности.',
deepen:'Углубиться',
deepenText:'Практическую идентифицируемость можно анализировать через сингулярные значения матрицы чувствительности, матрицу информации Фишера, профиль правдоподобия или байесовское апостериорное распределение. Разные методы отвечают на немного разные вопросы, но все показывают, насколько данные действительно ограничивают параметры.',
research:'Исследовательское замечание',
researchText:'Для рядов Прони особенно опасны перестановочная симметрия ветвей, близкие τᵢ и слишком большое число ветвей. Полезны упорядочивание τ₁<τ₂<..., логарифмическая параметризация положительных параметров, регуляризация и выбор сложности модели.',
