...
keyText:'Одна и та же параметризация должна выдерживать релаксацию, ползучесть, циклический отклик, предельные режимы при t→0 и t→∞, а также проверку неотрицательной диссипации.',
models:'Модель материала',
modelsText:'В лаборатории доступны редуцированные варианты модели Максвелла, стандартной линейной модели твёрдого тела, обобщённой модели Максвелла и модели с зависимым от состояния временем релаксации. Это учебные аналоги, которые сохраняют основные механизмы памяти и диссипации.',
protocols:'Протоколы нагружения',
protocolsText:'Релаксация проверяет спад напряжения при фиксированной деформации, ползучесть — рост деформации при фиксированном напряжении, циклический протокол — фазовый сдвиг и площадь петли.',
...
model:'модель',protocol:'протокол',maxwell:'Максвелл',sls:'модель Ценера',generalized:'обобщённая модель Максвелла',state:'зависимая от состояния',
...
pass:'норма',fail:'проверить',
warning:'ВАЖНО',
warningTitle:'Успех на одном протоколе не доказывает адекватность модели.',
warningText:'Параметры, отлично описывающие релаксацию, могут плохо предсказывать циклический отклик или ползучесть. Поэтому полноценная валидация должна быть многопротокольной.',
question:'ИССЛЕДОВАТЕЛЬСКОЕ ЗАДАНИЕ',
questionTitle:'Найди параметры, при которых модель хорошо описывает релаксацию, но заметно меняет диссипацию при циклическом нагружении.',
...
deepen:'Углубиться',
deepenText:'В реальном рабочем процессе лаборатория должна дополняться автоматизированными регрессионными тестами: единичным скачком деформации, единичным скачком напряжения, гармоническим частотным проходом, исследованием сходимости по Δt и проверкой дискретного баланса энергии.',
research:'Исследовательское замечание',
researchText:'Для нелинейных конечно-деформационных моделей полезно разделять верификацию и валидацию. Верификация отвечает на вопрос «правильно ли реализованы уравнения?», а валидация — «подходит ли эта модель конкретному материалу и эксперименту?».',
...
model:'model',protocol:'protocol',maxwell:'Максвелл',sls:'модель Ценера',generalized:'обобщённая модель Максвелла',state:'зависимая от состояния',
relaxation:'релаксация',creep:'ползучесть',cyclic:'циклическое нагружение',
